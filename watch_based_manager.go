...
func (i *objectCacheItem) stopIfIdle(now time.Time, maxIdleTime time.Duration) bool {
	...
	// to be retried.
	if !i.stopped && i.informer.HasSynced() && now.After(i.lastAccessTime.Add(maxIdleTime)) {
		return i.stopThreadUnsafe()
	}
	return false
}
...
func NewWatchBasedManager(
	...
	objectStore := NewObjectCache(listObject, watchObject, newObject, isImmutable, groupResource, clock.RealClock{}, maxIdleTime, handlerFuncs, wait.NeverStop)
